...
/**
 * This adapter modifies the generated SQL by adding a LIMIT and OFFSET clause
 * at the end of the generated SQL. This can be used to create a paginated
 * query.
 * 
 * LIMIT and OFFSET has limited support in relational databases, so this cannot
 * be considered a general solution for all paginated queries (and that is why
 * this adapter lives only in the test source tree and is not packaged with the
 * core library code).
 * 
...
public class LimitAndOffsetAdapter<R>
{
	...
	{
		return new LimitAndOffsetDecorator(selectModel.render(RenderingStrategy.MYBATIS3));
	}
